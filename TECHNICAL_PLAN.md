...
- **Re-probes authentication on state-change signals**, not once per process. A distinct successful-sign-in event also forces the Graph attempt that can clear sticky authorization suppression; ordinary state-change and suppress-details events remain payload-free accelerants and never make a fresh snapshot globally stale. The provider lives until its last widget is unpinned, so a single probe at startup means the ordinary flow never converges: a widget rendering sign-in-required launches the companion, the user signs in, and the provider holds its original result with a valid token in the broker. Re-probing is signal-driven with an overlap guard — no timer and no background service, per invariant 3 — and it re-probes in both directions rather than only while unauthenticated, so a status that can only improve is not left as a trap.

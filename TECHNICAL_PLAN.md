...
- Companion onboarding/settings/diagnostics. **Implemented in WinUI 3; installed-package measurement remains.** The packaged companion is single-instance, retains Sign in, Switch account, Sign out, interrupted-operation recovery, and metadata-free diagnostics, adds the privacy control and a New Outlook probe, and continues to pass the real window handle to WAM. The conversion has automated source checks and compiles, but it is not yet evidence about XAML rendering, activation redirection, or broker behaviour in the installed package.

...
- Companion onboarding/settings/diagnostics. **Next slice.** It also carries the diagnostics log below and the deferred settings-change refresh trigger.
- Adaptive Card 1.5 templates and data binding. **Done and measured on installed package 0.4.24.1.** `InboxCard` renders the committed snapshot; `SkeletonCard` is gone. Two constraints came from looking at the rendered card rather than from reasoning, and both are recorded in `docs/phase0-evidence.md`: the large size shows four rows rather than the cached five, because five plus the diagnostic block clipped the action row on a host that neither scrolls nor reports overflow; and read state is two `TextBlock`s selected by `$when` on a boolean rather than a value bound into the `weight` property, which rendered every sender identically. Treat binding into non-text Adaptive Card properties as unproven on this host.
...
- **Diagnostics move off the card and into a bounded log file.** The large card's diagnostic block is replaced by a metadata-free log — the existing operational events plus current delivery state — size-capped with rollover, written inside the package store so uninstall removes it, and opened from a button in the companion. This is a new persistent artifact and is approved as one; it is not telemetry, has no network path, and a test must hold that no sender, subject, account identifier, tenant identifier, or token can reach it. **The card block stays until the log and its companion button exist**, because production logging is metadata-free and in-memory, so removing it first would leave no way to observe provider state on a running machine.

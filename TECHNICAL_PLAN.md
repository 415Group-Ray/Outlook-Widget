...
**Authentication and the narrow production refresh are built and measured.** Gates 10 and 11 pass. Logout is implemented with suppress-first ordering and a durable signed-out marker, and its installed-package behavior passes on 0.4.19.0. Account switching is implemented with the same suppress-first boundary and a mutex-bound prior-cache-clear/new-selection commit, but it still needs installed-package measurement with a second account. Gate 12 passes in full: its query syntax, absence of an undocumented header requirement, and warm latency were measured on 2026-08-03, and the comparison of its returned count with New Outlook was taken on 2026-08-04 against installed package 0.4.22.0. That comparison could not be made exactly simultaneous — a mailbox in normal use changes faster than a manual two-window reading — so it rests on two observations that do not require simultaneity: the cached Focused count sat strictly below cached total unread, which rules out a filter silently returning the total, and New Outlook independently agreed on the Other count. `docs/phase0-evidence.md` records the method and its limit.

**`GraphMailClient` is called in production.** Stale activation, post-sign-in convergence, manual actions, and one provider-wide opportunistic five-minute timer while any widget instance is active run through `MailboxRefreshFetcher` and `RefreshCoordinator`; the validated snapshot is committed through `ProtectedCache`. Timer lifecycle and wiring have automated coverage and are measured on installed package 0.4.18.0: one refresh committed after 324.6 seconds active, while no refresh committed during 340.8 seconds after deactivation with the provider still alive. Privacy-setting changes now commit through `SettingsChangeCoordinator` and signal the provider to re-read state and deliver without an unnecessary Graph request. `docs/phase0-evidence.md` is authoritative for installed-package measurements.
...
- Supplies the real parent window handle that brokered sign-in is parented to. The WinUI window caches that handle on its dispatcher thread before interactive operations move to a worker, preserving the WAM parenting contract established by the earlier minimal Win32 probe.
...
- **Re-probes authentication on state-change signals**, not once per process. Interaction-required and approval-required both persist as authorization-suppression reasons, so a provider recycle cannot expose cached rows before the replacement process finishes its silent probe. A distinct successful-sign-in event also forces the Graph attempt that alone can clear sticky authorization suppression; ordinary state-change and suppress-details events remain payload-free accelerants and never make a fresh snapshot globally stale. The provider lives until its last widget is unpinned, so a single probe at startup means the ordinary flow never converges: a widget rendering sign-in-required launches the companion, the user signs in, and the provider holds its original result with a valid token in the broker. Re-probing is signal-driven with an overlap guard — no timer and no background service, per invariant 3 — and it re-probes in both directions rather than only while unauthenticated, so a status that can only improve is not left as a trap. The durable record retains the reason so cached-first recovery still renders the corresponding companion action; if its primary atomic replacement fails, a separate authorization fallback marker preserves that decision and is intentionally outside generic interrupted-operation recovery.
...
- `StateChangeListener`: creates the two named notification events and turns either signal into a callback. Required rather than optional: the signalling side opens the events by name and treats absence or operational open failure as an undelivered accelerant, so without something creating them every cross-process signal is a silent no-op.
- `SelectedAccountStore`: records which MSAL home account the user actually chose, published by the companion on a successful interactive acquisition and read by `SilentAuthService` in both processes. **Publication is a state commit, not a file write**, and the store's own mutex-acquiring `Write` is `internal` to keep it that way: the cached mailbox belongs to whichever account was selected when it was committed, so replacing the identifier without deciding what happens to that snapshot in the same critical section leaves the previous account's mail on screen under the new selection. `CommitInteractiveSelectionAction` owns that decision for sign-in and makes it under the held lock, because a refresh scoped to the prior selection is legitimate until the identifier changes; `CommitAccountSwitchStateAction` and `CommitSignedOutStateAction` own it for the two disclosure-reducing operations. Its own file rather than part of the snapshot, because silent acquisition needs it *before* any refresh has succeeded and a cleared snapshot must not take it with it. DPAPI-protected, as step 6 of section 4 requires; it holds an opaque directory object ID and the registration it is scoped to, with no mailbox content, no user principal name, and no token. **It fails closed on anything but a clean absence.** A recorded selection that is no longer cached produces interaction-required rather than a fallback to another account, and so does a record that exists and cannot be trusted — unreadable, corrupt, undecryptable, or carrying no identifier. A genuinely missing file permits the first-cached-account fallback **only while exactly one account is cached**: with more than one there is nothing to distinguish a fresh install from a sign-in whose selection failed to persist, so the ambiguity itself refuses. Collapsing "unreadable" into "no selection" is the specific mistake here, because on a machine with more than one cached account it reads a different mailbox and looks exactly like success; this store's direction is therefore the **opposite** of `AuthorizationStateStore`'s, where an unreadable record must yield no refinement because over-claiming withdraws a retry the user may want. Same shape of question, opposite harms.
...
- `StateChangeSignal` and `SignInCompletedSignal`: raise separate state-changed and successful-sign-in events through the shared best-effort named-event helper, so missing-event, I/O, and access-denied handling exists once for every notification. `StateCommitCoordinator` still raises the general event only after a successful commit. The companion raises the distinct sign-in event only after successful authentication; that event is the narrow evidence that may force Graph despite a fresh same-account snapshot. Without it a pinned widget can retain sticky authorization suppression until unpinned, while treating every general event as the same evidence would let privacy or suppress-first operations refresh the prior account.
...
1. Write this operation's disclosure tombstone, register its owning handle, and attempt to set the suppress-details event. The durable marker is the means to fail closed; the event only accelerates provider convergence and an operational signalling failure does not undo publication.
2. Call MSAL `RemoveAsync` for the selected account.
3. Set an explicit local signed-out state so the provider does not immediately reacquire the Windows operating-system account silently, and commit it under the bounded mutation mutex. Within that multi-file local commit, replace the selected account identifier last: an authorization or cache mutation failure must retain the original selection so a retry remains scoped to that account rather than removing every account in the app-local MSAL cache. Write that final record through a temporary file and atomic replace too, so its own I/O failure preserves the prior complete identifier.
...
- **Mutation mutex — a real named `Mutex`, never held across an `await`, always acquired with a bounded wait.** Its critical section is entirely synchronous local work: DPAPI protect/unprotect, temp-file write, atomic replace, generation increment. Acquire and release happen on one thread with no suspension point in between, which is exactly the shape `Mutex` supports. `AbandonedMutexException` handling still applies, because a process can be killed mid-commit.
  ...
  **Every acquisition uses `WaitOne(timeout)`. The parameterless `WaitOne()` overload is prohibited** — it waits indefinitely, so a peer wedged inside a critical section would hang the caller with no recovery path, which is precisely the failure the lock-free read design exists to prevent. Use a 2-second timeout: the critical section is local synchronous I/O whose worst case is the bounded 25/50/100 ms replace retry, so 2 seconds is generous by an order of magnitude and any timeout indicates a genuinely stuck peer rather than normal contention.
  ...
  - **Logout, account switch, privacy change, and cache clear:** these must not silently no-op. A logout whose commit was skipped would leave the previous account's subjects on screen, which is a privacy failure rather than a stale-data annoyance. Retry once, and if the second attempt also times out, surface an explicit failure in the companion, keep the marker fail-closed, and direct the user to explicit marker recovery followed by a provider recycle if contention persists. Closing the Widgets Board only deactivates the pinned widget and does not release its provider or mutex. Suppression of message details in that window does **not** depend on this mutex; see the disclosure tombstone below.
...
1. On user intent, the companion writes its operation's tombstone file and attempts to set a named suppress-details event. Neither takes the mutation mutex. The event is only an accelerant over authoritative disk state: absence, I/O failure, or access denial is an undelivered notification rather than a failed publication. Once the marker is visible, publication either returns its registered owning handle or unregisters the operation without deleting the marker on every exception before return.
...
- **Refresh lease — a record, not a held lock.** Cross-process single-flight is expressed as a small lease record (owner process ID, owner instance GUID, expiry, and the known cache generation at claim) written under a brief synchronous hold of the mutation mutex. The starting generation lets a peer prove a commit even when it wakes after the cache write but before lease release; an unknown or legacy value cannot prove completion. Nothing is held while the refresh runs. Acquiring means: take the mutex, see whether a live unexpired lease exists, read the generation and write your own record if not, release. Releasing means: take the mutex, clear the record if you still own it, release. Both operations are short, synchronous, and single-threaded.
...
- Named state-change, successful-sign-in, and suppress-details events are best-effort convergence accelerants. Missing-event, I/O, and access-denied failures never override authoritative disk state, fail an otherwise completed mutation, or leave a published marker registered after its operation has stopped.
...
- [.NET `EventWaitHandle.OpenExisting`](https://learn.microsoft.com/en-us/dotnet/api/system.threading.eventwaithandle.openexisting?view=net-10.0)
...
- WAM authentication/account lifecycle, including logout and account switch end to end. **Implemented; account-switch device evidence remains.** Acquisition is complete both ways — interactive in the companion, silent in the provider, with a shared token cache and failure classification — and gates 8 and 9 measured it. **Logout is implemented and measured on installed package 0.4.19.0:** it publishes its signed-out tombstone before awaiting app-local MSAL cache removal, commits a protected signed-out marker with the selected identifier removed and mailbox state cleared, then clears only its own tombstone. The reference-machine proof observed cleared generation 20, no mailbox payload or retained identifier, no leftover tombstone, and no cache or generation change after provider activation. The marker prevents the provider from treating post-logout state as a fresh install and silently reacquiring the Windows operating-system account. **Account switching is implemented but not yet measured with a second account:** the companion publishes the same signed-out suppression before forcing WAM's account picker, then clears the prior mailbox snapshot and replaces the selected identifier together under the mutation mutex before lifting only its own tombstone. Same-account selection is not accepted as proof of cross-account cache isolation. **Sign-in publishes its selection the same atomic way, and did not originally:** it replaced the identifier alone, so a sign-in landing on a different account than the one recorded left that account's snapshot committed and unsuppressed. `CommitInteractiveSelectionAction` now removes the cached mailbox in the same critical section unless the cache is positively absent or already cleared, and retains it when the account is unchanged so an expired token does not blank the widget. The commit path is measured on installed package 0.4.22.0 — signing in from the signed-out state left by the logout measurement published the identifier and then committed a Graph-backed snapshot, advancing the generation 20 → 22 with no orphaned suppression marker — but the cross-account case the fix was written for was not reachable, because the prior cache was already cleared. That case has automated coverage only and stays unmeasured until a second account exists.
...
- Companion onboarding/settings/diagnostics. **Implemented in WinUI 3; rendering and activation redirection are installed-package measured.** The packaged companion is single-instance, retains Sign in, Switch account, Sign out, interrupted-operation recovery, and metadata-free diagnostics, adds the privacy control and a New Outlook probe, and continues to pass the real window handle to WAM. Installed packages 0.6.31.0 and 0.6.32.0 rendered a responsive XAML window, and a secondary activation retained one process and window. Interactive WAM was not invoked, so broker behaviour with the WinUI parent handle remains unmeasured.
- Adaptive Card 1.5 templates and data binding. **The cached rendering is measured on installed package 0.4.24.1; the latest layout adjustment is not.** `InboxCard` renders the committed snapshot; `SkeletonCard` is gone. The measured layout showed four rows because five plus the former diagnostic block clipped the action row. The diagnostic block has since moved to the bounded log and companion, so source now restores the approved fifth large row; its fit must be checked on the Widgets host before it is called measured. Read state remains the proven pair of `TextBlock`s selected by `$when` on a boolean. Treat binding into non-text Adaptive Card properties as unproven on this host.
- Small, medium, large, counts-only, signed-out, loading, stale, and error states. **All are now built; loading and error still need installed-host observation.** Refresh presentation is transient provider state over the last validated snapshot: ordinary loading, throttling, timeout, offline, invalid-response, and service failures keep cached content; authorization-invalidating outcomes withhold message rows and offer the companion. A peer-held refresh self-clears to the documented unknown state after the lease ends without a commit signal. The 24-hour `StaleDetailSuppression` rule gained its first implementation in the card slice — the constant had existed since Phase 1 with nothing consulting it, because nothing rendered a subject to withhold.

**Four scope decisions taken 2026-08-04, recorded here so they do not drift into implementation unnoticed:**

- **Opening an individual message stays in v1, and its safety test will be narrowed to allow it.** `ProviderCardTests.No_provider_source_names_a_mailbox_field_outside_the_approved_set` currently forbids the cached link's Graph property name anywhere in provider source, which is stricter than section 8's approved cache contents and would block section 9's message action. When that slice lands the check narrows from "no provider file" to "no card or template file", preserving what it actually protects — no link and no message identifier in Adaptive Card JSON or `CustomState`, per section 17 — while letting the provider validate the link against the Outlook-host allowlist as section 3 requires. Narrowing it is an approved scope change, not a convenience: it must be done deliberately, in the slice that needs it, and never by deleting the assertion.
- **Diagnostics moved off the card and into a bounded log file.** The large card's diagnostic block was replaced by a metadata-free log — the existing operational events plus current delivery state — size-capped with rollover, written inside the package store so uninstall removes it, and opened from a button in the companion. It is not telemetry and has no network path. Tests hold that no sender, subject, account identifier, tenant identifier, or token can reach it. The original Win32 diagnostics button was measured before removal; the WinUI replacement still needs its installed-package check.
- **Cross-account isolation is a permanent v1 limitation.** No second account exists on the reference machine and none will be obtained, so account switching, foreign-cache removal on sign-in, and the multi-account silent-acquisition refusal branches ship with automated coverage only. The implementation is retained rather than cut. This must be stated in the README as a known limitation and never described as measured.
- **The interim app icon ships.** Phase 2 does not block on artwork being designed outside this repository; the icon is swapped when it lands. It remains an open item rather than a settled one.

...
- `SelectedAccountStore`: records which MSAL home account the user actually chose, written by the companion on a successful interactive acquisition and read by `SilentAuthService` in both processes. Its own file rather than part of the snapshot, because silent acquisition needs it *before* any refresh has succeeded and a cleared snapshot must not take it with it. Not DPAPI-protected: an opaque directory object ID and the registration it is scoped to, with no mailbox content, no user principal name, and no token — the snapshot protects the same identifier only because it rides alongside senders and subjects. **It fails closed on anything but a clean absence.** A recorded selection that is no longer cached produces interaction-required rather than a fallback to another account, and so does a record that exists and cannot be trusted — unreadable, corrupt, or carrying no identifier. Only a genuinely missing file permits the first-cached-account fallback. Collapsing "unreadable" into "no selection" is the specific mistake here, because on a machine with more than one cached account it reads a different mailbox and looks exactly like success; this store's direction is therefore the **opposite** of `AuthorizationStateStore`'s, where an unreadable record must yield no refinement because over-claiming withdraws a retry the user may want. Same shape of question, opposite harms.

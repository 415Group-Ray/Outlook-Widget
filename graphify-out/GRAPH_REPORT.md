...
## Corpus Check
- 98 files · ~142,328 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1256 nodes · 2884 edges · 56 communities (52 shown, 4 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 326 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `89f2699a`
...
2. `OutlookWidget.Core.Refresh` - 36 edges
3. `SelectedAccountTests` - 34 edges
4. `CompanionWindow` - 31 edges
5. `IOperationalLogger` - 28 edges
6. `OutlookWidget.Core.Caching` - 27 edges
7. `OutlookWidget.Core.Diagnostics` - 27 edges
...
Cohesion: 0.08
Nodes (17): Fact, Func, TimeSpan, DeliveryWorkerTests, Fact, MutationMutexTests, Fact, ProtectedCacheTests (+9 more)
...
Cohesion: 0.07
Nodes (23): EventWaitHandle, IDisposable, Process, Queue, RefreshWork, RefreshTrigger, Action, bool (+15 more)
...
Cohesion: 0.06
Nodes (25): Exception, IdentityKind, IPublicClientApplication, Task, Program, AuthenticationConfigurationResult, StateChangeSignal, Func (+17 more)
...
Cohesion: 0.06
Nodes (36): ImmutableArray, AuthenticationOptions, DateTimeOffset, Guid, JsonSerializerOptions, AuthorizationRecord, AuthorizationStateStore, Func (+28 more)
...
### Community 53 - "RefreshLeaseStore"
Cohesion: 0.19
...
- **Why does `IOperationalLogger` connect `IOperationalLogger` to `AuthenticationOutcomeTests`, `.RefreshAsync`, `DisclosureTombstoneStore`, `StateCommitCoordinator.cs`, `GraphMailClient`, `WidgetProvider`, `AuthenticationConfigurationTests`, `OutlookLauncher`, `DeliveryWorker`, `MutationLock`, `ProviderRefreshWorker`, `WidgetInstanceRegistry`, `CompanionLauncher`, `RefreshLeaseStore`, `.Record`, `CoordinationPaths`, `SelectedAccountTests`?**
  _High betweenness centrality (0.122) - this node is a cross-community bridge._
- **Why does `GraphMailClient` connect `GraphMailClient` to `.ReadAsync`, `OutlookWidget.Core.Refresh`, `IOperationalLogger`, `ProviderRefreshWorker`, `.Combine`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `OutlookWidget.Core.Refresh` connect `OutlookWidget.Core.Refresh` to `DisclosureTombstoneStore`, `StateCommitCoordinator.cs`, `.Current`, `MutationLock`, `CoordinationStaticAnalysisTests`, `RefreshLeaseStore`?**

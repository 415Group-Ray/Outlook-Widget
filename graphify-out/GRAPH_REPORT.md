...
## Corpus Check
- 126 files · ~174,163 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1672 nodes · 3927 edges · 90 communities (85 shown, 5 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 501 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0cdf4a85`
...
Cohesion: 0.22
Nodes (11): Clear interrupted operations recovery control, Real cross-process concurrency test suite, No telemetry, metadata-free local logs, Sign-in publishes account and mailbox decision together, Account switching flow, Adaptive Cards schema 1.5 rendering, Counts-only privacy rendering, Disclosure tombstone (suppress-first) (+3 more)

### Community 34 - ".Read"
Cohesion: 0.09
Nodes (23): ImmutableArray, AuthenticationOptions, DateTimeOffset, Guid, JsonSerializerOptions, AuthorizationRecord, AuthorizationStateStore, Func (+15 more)

### Community 35 - "OutlookWidget.Provider widget provider"
Cohesion: 0.12
Nodes (25): AGENTS.md agent guidance, Upgrades require -ForceApplicationShutdown, graphify working agreement, Repository map (project layout), Scope and phase gate discipline, Sources of truth ordering, No access or refresh token persisted, BrokerClient.NoParentWindow named member (+17 more)
...
Cohesion: 0.18
Nodes (11): Delivery Stays Outside the Refresh Transaction, Provider Delivery Convergence, Durable Tombstone Is Authoritative; Named Events Are Best-Effort Accelerants, Deferred Intune/RMM deployment design, Opportunistic five-minute active refresh timer, Phased implementation roadmap, Twelve-step refresh algorithm, Refresh transaction bounds and lease horizon (+3 more)
...
Cohesion: 0.15
Nodes (15): Approved Graph field set boundary, IRefreshFetcher (production implementation over silent auth plus GraphMailClient), MailboxReadout, RefreshPayload, Supported environment baseline, Single-tenant Entra app registration, Optional Focused unread count query, GraphMailClient (+7 more)
...
### Community 79 - "TokenAcquisitionStatus"
Cohesion: 0.29
...
- **Why does `IOperationalLogger` connect `IOperationalLogger` to `.ReadAsync`, `.RefreshAsync`, `DisclosureTombstoneStore`, `SilentAuthProbe`, `ActiveRefreshTimer`, `.Record`, `AuthenticationConfigurationTests`, `SelectedAccountStore`, `SettingsChangeTests`, `.AcquireAsync`, `MutationLock`, `MutationMutex`, `CoordinationFixture`, `OutlookLauncher`, `.Read`, `DeliveryWorker`, `StateChangeListener`, `CompanionLauncher`, `WidgetInstanceRegistry`, `ProtectedCache`, `WidgetProvider`?**
